...
| Frontend artifacts (Immiglance — Landing + Client) | ~55 relevant / 79 total files scanned | Scanned while these were one app (`Immiglance/Frontend`); that app has since been split into `Immiglance/Landing` (public + pre-auth) and `Immiglance/Client` (authenticated portal), with no file added or removed by the split, so the counts still hold — only the paths changed. 24 purely presentational components excluded with reason (see Immiglance extraction agent's stated exclusions). |
| Frontend artifacts (Admin, formerly INSZoom) | ~26 relevant / 28 total files scanned | 2 excluded (ErrorBoundary, QuestionnaireAnswersPanel) with reason. |

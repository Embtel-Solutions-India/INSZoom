...
function statusLabel(caseData) {
  const key = String(caseData?.stage || caseData?.status || "").toLowerCase();
  return CASE_STATUS_LABELS[key] || "Evaluation In Progress";
}
      ...
      >
        <div className="h-16 flex items-center gap-2.5 px-4 border-b border-sidebar-border">
          <Link to="/" className="flex items-center gap-2.5 no-underline min-w-0">
            ...
            {!collapsed && <span className="font-bold text-sidebar-foreground truncate">BAIS</span>}
          </Link>
        </div>

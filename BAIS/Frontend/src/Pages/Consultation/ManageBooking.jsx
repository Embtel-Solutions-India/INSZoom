...
export default function ManageBooking() {
  ...
  if (bookingQuery.isLoading) {
    return <EligibilityShell><div className="max-w-md mx-auto px-5 py-16"><div className="h-48 rounded-2xl bg-muted animate-pulse" /></div></EligibilityShell>;
  }
    ...
    return (
      <EligibilityShell>
        <div className="max-w-md mx-auto px-5 py-24 text-center">
          <p className="font-bold text-foreground mb-2">This booking link is invalid or has expired</p>
          <p className="text-sm text-muted-foreground">If you still need a consultation, please start a new booking.</p>
        </div>
      </EligibilityShell>
    );
  }
    ...
    <EligibilityShell>
      <div className="max-w-lg mx-auto px-5 sm:px-6 py-14">
        <div className="rounded-2xl border border-border p-6 sm:p-8">
          {mode === "view" && (
            <>
              <p className={`text-xs font-bold uppercase tracking-wider mb-3 ${booking.status === "cancelled" ? "text-destructive" : "text-accent-foreground"}`}>
              ...
              <p className="text-sm text-muted-foreground mb-6">with {booking.publicHostName} &middot; {booking.locationType === "phone" ? "Phone call" : "Video call"}</p>

              {booking.status !== "cancelled" && (
                <div className="flex flex-col sm:flex-row gap-3">
                  <button type="button" onClick={() => setMode("reschedule")} className="flex-1 px-5 py-3 rounded-xl border border-border text-foreground font-bold text-sm hover:bg-secondary cursor-pointer">
                    Reschedule
                  </button>
                  <button type="button" onClick={() => setMode("cancel-confirm")} className="flex-1 px-5 py-3 rounded-xl border border-destructive/30 text-destructive font-bold text-sm hover:bg-destructive/10 cursor-pointer">
                    Cancel
                  </button>
                </div>
              )}
          ...
          {mode === "reschedule" && (
            <>
              <button type="button" onClick={() => setMode("view")} className="text-xs font-bold text-muted-foreground hover:text-foreground mb-4 cursor-pointer">&larr; Back</button>
              <p className="text-sm font-bold text-foreground mb-4">Pick a new time</p>
          ...
          {mode === "cancel-confirm" && (
            <>
              <p className="font-bold text-foreground mb-2">Cancel this consultation?</p>
              <p className="text-sm text-muted-foreground mb-4">Let us know why (optional), so we can follow up appropriately.</p>
              <textarea value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} rows={2} className="w-full rounded-xl border border-border px-4 py-3 text-sm mb-4" placeholder="Reason (optional)" />
              <div className="flex gap-3">
                <button type="button" onClick={() => setMode("view")} className="flex-1 px-5 py-3 rounded-xl border border-border text-foreground font-bold text-sm cursor-pointer">
                  Keep booking
                </button>
                <button
                  type="button"
                  ...
                  className="flex-1 px-5 py-3 rounded-xl bg-destructive hover:opacity-90 text-destructive-foreground font-bold text-sm cursor-pointer"
                >

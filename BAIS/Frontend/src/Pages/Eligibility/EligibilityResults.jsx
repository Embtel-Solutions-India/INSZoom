    ...
    return (
      <EligibilityShell>
        <div className="max-w-lg mx-auto px-5 py-24 text-center">
          <p className="font-bold text-foreground mb-2">Your results aren't available here</p>
          ...
          <button
            ...
            className="inline-flex items-center justify-center px-6 py-3 rounded-xl bg-primary hover:opacity-90 text-primary-foreground font-bold text-sm transition cursor-pointer"
          >
    ...
    <EligibilityShell>
      <div className="max-w-2xl mx-auto px-5 sm:px-6 py-14 sm:py-20">
        <div className={`rounded-2xl border ${meta.ring} ring-4 ${meta.bg} p-6 sm:p-8 mb-8 text-center`}>
          <span className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full ${meta.text} bg-card text-xs font-bold uppercase tracking-widest mb-4`}>
            <span className={`w-2 h-2 rounded-full ${meta.color}`} aria-hidden="true" />
            {meta.label}
          </span>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-foreground mb-2">{result.pathwayString}</h1>
          <p className="text-muted-foreground text-sm">{result.nextStep}</p>
        </div>

        <div className="mb-8">
          <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-3">Your evidence strength</p>
          <div className="space-y-2.5">
            {(result.evidenceStrength || []).map((e) => (
              <div key={e.key} className="flex items-center gap-3">
                <span className="text-xs text-muted-foreground w-40 shrink-0 truncate">{e.key.replace(/_/g, " ")}</span>
        ...
        {result.alternativePathways?.length > 0 && (
          <div className="mb-10 rounded-2xl border border-border p-5 sm:p-6">
            <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-3">Pathways worth exploring</p>
            <div className="flex flex-wrap gap-2">
              {result.alternativePathways.map((p) => (
                <span key={p} className="px-3 py-1.5 rounded-full bg-secondary text-secondary-foreground text-xs font-semibold">{p}</span>
              ))}
            </div>
          </div>
        )}
        ...
        <button
          ...
          className="w-full inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl text-primary-foreground font-bold text-base transition active:scale-95 cursor-pointer"

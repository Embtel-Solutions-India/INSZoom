...
const uscisFormTemplateSchema = new mongoose.Schema(
  {
    ...
    // "biographic_active" - non-destructive additive value (existing docs
    // unaffected, no migration needed): a template whose ONLY reviewed
    // mapping is the high-confidence biographic-core subset generated by
    // BiographicMappingService, not a full curated crosswalk. Deliberately
    // distinct from "active" - VersionManagementService.activate's Gate 2
    // (mappingStatus !== "active") keeps correctly refusing full production
    // activation for these until a human completes real mapping review;
    // this tier only unlocks AutoFillService's biographicFallback path.
    mappingStatus: { type: String, enum: ["unmapped", "draft", "active", "needs_review", "archived", "biographic_active"], default: "unmapped", index: true },

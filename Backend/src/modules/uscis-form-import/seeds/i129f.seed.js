// node src/modules/uscis-form-import/seeds/i129f.seed.js
// (or: npm run seed:i129f, from Backend/)
//
// Imports (if not already present) and activates the bundled I-129F
// (edition 2025-01-20) so uscis-form.service.js's
// latestTemplatesByAssignmentRules()/templateAppliesToCase() will assign it
// to a K-1 case. Idempotent and non-destructive.
...
const TITLE = "Petition for Alien Fianc\u00e9(e) (I-129F)";
const DEFAULT_FILE = path.resolve(__dirname, "../../../../dev-assets/uscis/i-129f_2025-01-20.pdf");
...
async function seedI129FTemplate({ file } = {}) {
  ...
  template.visaTypes = Array.from(new Set([...(template.visaTypes || []), "K-1", ...registryVisaTypes]));
...
module.exports = seedI129FTemplate;

if (require.main === module) {
  mongoose
    .connect(env.mongoUri)
    .then(() => seedI129FTemplate({}))

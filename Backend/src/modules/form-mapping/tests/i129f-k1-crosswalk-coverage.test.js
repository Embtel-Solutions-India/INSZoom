// DB-free companion test for the K-1 I-129F crosswalk, mirroring
// i129-l1a-crosswalk-coverage.test.js's approach: normalizes the bundled
// I-129F PDF locally via pdf-lib (the same asset i129f.seed.js imports from)
// so mapping correctness can be verified without a database connection.
...
const { classifyField, MAPPED_EDGES } = require("../config/i129f-k1-crosswalk");
const k1 = require("../../family-workflow/questionnaires/k1");

const PDF_PATH = path.resolve(__dirname, "../../../../dev-assets/uscis/i-129f_2025-01-20.pdf");
...
test("every real I-129F field classifies to a known status, and every mapped edge is grounded in a real field name", async (t) => {
  let fields;
  try {
  ...
  assert.ok(fields.length >= 400, `expected the real I-129F PDF's fillable field count (~445), got ${fields.length}`);
  ...
  assert.deepEqual(badFieldNames, [], "every crosswalk edge must target a real field on the actual I-129F template");
...
test("every crosswalk edge's source key is a real k1.js fieldCatalog key", () => {
  ...
  const badSources = MAPPED_EDGES
    ...
    .map((match) => match[1]);
  assert.deepEqual(badSources, [], "every raw.questionnaireAnswers source must resolve to a real k1.js fieldCatalog() question key");
});

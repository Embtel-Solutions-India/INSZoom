// node src/modules/form-mapping/seeds/i129f-k1-mapping.seed.js
// (or: npm run seed:i129f-k1-mapping, from Backend/)
//
// Converts the reviewed crosswalk (../config/i129f-k1-crosswalk.js) into a
// USCISMappingVersion graph for the active I-129F template and activates it.
// Mirrors i129-h1b-mapping.seed.js's exact mechanics (idempotent via content
// checksum, version N+1 on a changed crosswalk). Unlike the I-129 seed, this
// one is genuinely independent - I-129F is its own template, not shared with
// any other visa type in this codebase, so there's no single-active-version
...
const { classifyField, MAPPED_EDGES, FORM_CODE, VERSION } = require("../config/i129f-k1-crosswalk");
...
// Same workaround as i129-h1b-mapping.seed.js's contentChecksum - see that
// file's comment for why the stored .checksum can't be trusted directly.
function contentChecksum(graph) {
  const { mappingVersion, ...rest } = graph || {};
  return MappingGraphService.graphChecksum(rest);
}

async function seedI129fK1Mapping({ user } = {}) {
  const template = await USCISFormTemplate.findOne({ formCode: FORM_CODE, version: VERSION });
  if (!template) {
    const error = new Error(`No USCISFormTemplate found for ${FORM_CODE} ${VERSION} - run seed:i129f first.`);
    error.code = "I129F_TEMPLATE_NOT_FOUND";
    throw error;
  }
  if (template.status !== "active") {
    const error = new Error(`USCISFormTemplate ${FORM_CODE} ${VERSION} is not active (status: ${template.status}) - seed:i129f must complete first.`);
    error.code = "I129F_TEMPLATE_NOT_ACTIVE";
    throw error;
  }
...
module.exports = seedI129fK1Mapping;

if (require.main === module) {
  mongoose
    .connect(env.mongoUri)
    .then(() => seedI129fK1Mapping({}))
    .then(({ template, mappingVersion, classification }) => {
      console.log("I-129F K-1 mapping seeded and activated.");
    ...
    .then(() => process.exit(0))
    .catch(async (error) => {
      console.error("Failed to seed I-129F K-1 mapping:", error.message);

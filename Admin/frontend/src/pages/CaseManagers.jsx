...
const CaseManagers = () => {
  ...
  const getStatusBadge = (status) => {
    return status === 'Active' ? (
      ...
      <span className="px-2 py-1 text-xs font-semibold bg-secondary text-foreground rounded-full">Inactive</span>
    )
  }
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Case Managers</h1>
          <p className="text-muted-foreground mt-1">Manage and monitor case manager performance</p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
          <div className="bg-card rounded-lg shadow p-4 border-l-4 border-blue-500">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Total Case Managers</p>
                <p className="text-2xl font-bold text-foreground">{stats.totalCaseManagers}</p>
              </div>
          ...
          <div className="bg-card rounded-lg shadow p-4 border-l-4 border-green-500">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Active Case Managers</p>
                <p className="text-2xl font-bold text-foreground">{stats.activeCaseManagers}</p>
              </div>
          ...
          <div className="bg-card rounded-lg shadow p-4 border-l-4 border-purple-500">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-foreground">{stats.totalActiveCases}</p>
              </div>
          ...
          <div className="bg-card rounded-lg shadow p-4 border-l-4 border-orange-500">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Total Completed Cases</p>
                <p className="text-2xl font-bold text-foreground">{stats.totalCompletedCases}</p>
              </div>
          ...
          <div className="bg-card rounded-lg shadow p-4 border-l-4 border-blue-500">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Total Revenue Managed</p>
                <p className="text-2xl font-bold text-foreground">${(stats.totalRevenueManaged || 0).toLocaleString()}</p>
              </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-border">
            ...
            <tbody className="bg-card divide-y divide-border">
              {caseManagers.map((cm) => (
                <tr key={cm._id} className="hover:bg-muted cursor-pointer" onClick={() => navigate(`/case-managers/${cm._id}`)}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        <div className="text-sm font-medium text-foreground">{cm.name}</div>
                        <div className="text-sm text-muted-foreground">ID: {cm._id?.slice(-6).toUpperCase()}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <div className="text-sm font-semibold text-foreground">${(cm.totalRevenue || 0).toLocaleString()}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <div className="flex items-center justify-center gap-1">
                      <Briefcase className="w-4 h-4 text-muted-foreground" />
                      <span className="text-sm font-semibold text-foreground">{cm.currentWorkload}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">
      ...
      {!loading && caseManagers.length === 0 && (
        <div className="bg-card rounded-lg shadow p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-foreground mb-2">No case managers found</h3>

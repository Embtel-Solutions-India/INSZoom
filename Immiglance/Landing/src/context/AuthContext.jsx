...
export function AuthProvider({ children }) {
  ...
  const verifySession = useCallback(async (autoRetry = true) => {
    ...
    try {
      ...
      if (err?.status === 401) {
        ...
        // already cleared it and fired immiglance:session-expired by this point;
  ...
  useEffect(() => {
    ...
    window.addEventListener("immiglance:session-expired", handler);
    return () => window.removeEventListener("immiglance:session-expired", handler);
  }, [clearSession]);
